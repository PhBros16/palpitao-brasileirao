...
export async function buscarTrofeusJogador(participantId: string): Promise<TrofeusDoJogador> {
  ...
  for (let i = 0; i < history.length; i++) {
    ...
    // Palpites por rodada — empates, goleadas, 0x0, 2x1
  ...
  const mediaPts = rodadasJogadas > 0 ? totalPts / rodadasJogadas : 0

  // ─── Comparações com o grupo ───────────────────────────────────────────

  // Média de cada participante (pra melhorMedia)
  ...
  // Rodadas em 1º de cada participante (pra líder absoluto)
  ...
  function saldosNaRodada(r: Rodada): number {
    let cnt = 0
    for (const p of r.palpitesJogador) {
      if (p.home_score === null || p.away_score === null) continue
      if (p.pred_h === p.home_score && p.pred_a === p.away_score) continue
      if (p.pred_h - p.pred_a === p.home_score - p.away_score) cnt++
    }
    return cnt
  }
  ...
  const tier1: TrofeuReal[] = [
    ...
    { id: 't1-pacifista', icon: '🏳️', tier: 1, label: 'O Pacifista',
    ...
    { id: 't1-zero-a-zero', icon: '⚰️', tier: 1, label: 'Zero a Zero',
      desc: 'Apostou 0x0 em 3+ jogos ao longo da competição.',
      unlocked: apostou0x0Count >= 3 },

    { id: 't1-galinha', icon: '🐔', tier: 1, label: 'Galinha',
    ...
    { id: 't1-dormiu', icon: '😴', tier: 1, label: 'Dormiu no Ponto',
      desc: 'Perdeu o prazo de palpite em 3+ rodadas.',
      unlocked: faltouPalpitar >= 3 },

    { id: 't1-monolito', icon: '🗿', tier: 1, label: 'O Monólito',
  ...
  const tier2: TrofeuReal[] = [
    ...
    { id: 't2-sangue-frio', icon: '🧊', tier: 2, label: 'Sangue Frio',
    ...
    { id: 't2-diplomata', icon: '🤝', tier: 2, label: 'Diplomata',
      desc: 'Apostou mais empates (%) que qualquer outro (mín. 3 rodadas).',
      unlocked: rodadasJogadas >= 3 && participantId === maisEmpatesId },

    { id: 't2-showman', icon: '🎪', tier: 2, label: 'O Showman',
      desc: 'Acertou um placar com 5+ gols no total.',
      unlocked: history.some((r) => r.palpitesJogador.some((p) =>
        p.pred_h === p.home_score && p.pred_a === p.away_score && (p.pred_h + p.pred_a) >= 5
      )) },

    { id: 't2-lanterninha', icon: '💩', tier: 2, label: 'Lanterninha Raiz',
      desc: 'Ficou 3+ rodadas em último lugar.',
      unlocked: rodadasUltimo >= 3 },

    { id: 't2-fenix', icon: '🔄', tier: 2, label: 'Fênix',
  ...
  const tier3: TrofeuReal[] = [
    { id: 't3-perfeicao', icon: '💎', tier: 3, label: 'Perfeição',
    ...
    { id: 't3-lider-absoluto', icon: '🏆', tier: 3, label: 'Líder Absoluto',
    ...
    { id: 't3-relampago', icon: '⚡', tier: 3, label: 'Relâmpago',
      desc: '7+ placares exatos ao longo de toda a competição.',
      unlocked: cravadas >= 7 },

    { id: 't3-predador', icon: '🦅', tier: 3, label: 'O Predador',
    ...
    { id: 't3-invicto', icon: '🎖️', tier: 3, label: 'Invicto',
    ...
    { id: 't3-saldo-perfeito', icon: '🧿', tier: 3, label: 'Saldo Perfeito',
      desc: 'Acertou o saldo de gols em 10+ jogos na competição.',
      unlocked: saldos >= 10 },

    { id: 't3-vidente', icon: '🔮', tier: 3, label: 'Vidente',
    ...
    { id: 't3-implacavel', icon: '🧲', tier: 3, label: 'Implacável',
      desc: '5+ rodadas consecutivas no top 3.',
      unlocked: maxConsecTop3 >= 5 },

    { id: 't3-franco-atirador', icon: '🎯', tier: 3, label: 'Franco Atirador',
  ...
  const tier4: TrofeuReal[] = [
    { id: 't4-campeao', icon: '👑', tier: 4, label: 'CAMPEÃO!',
      desc: 'O maior pontuador de toda a competição. Eterno.',
      unlocked: history.length >= 3 && participantId === liderTotalId && (totalPorPart.get(liderTotalId) ?? 0) > 0 },
  ]
